...
1. **The hero has no CTA — it has one exit.** No enquiry pill, no button: only `.h-scroll` ("See the work", a 34px gold rule plus a mono label), and it goes to `/portfolio`, not `#work`, so the hero's single affordance lands on the full gallery through the same cross-document `@view-transition` as the work gate. The always-available quiet contact is the nav's Instagram/Facebook icons; the loud ask is `#book`, which sits after Reviews and FAQ in the markup. Do not re-add a hero button or a sticky enquiry bar.
...
- The expertise rows reuse the FAQ accordion wholesale: `<li class="ei">` with a hairline `border-top`, a `<button class="ei-trigger">` (title + circular `.ei-toggle` that CSS-rotates 45° when open), an absolutely positioned `.ei-rule` gold sweep whose width GSAP animates (60% open, 30% hover), and the `height: 0` panel measured and tweened exactly like `.fq-panel` (0.55 expo.out / 0.35 expo.in). There are no per-skill summary lines, no numerals (numbering three styles reads as a ranking), no card border and no hover timeline — the old GSAP `x`/`color` timeline was the only writer of inline colours in the section, which is why the theme toggle no longer needs a `clearProps` call. The accordion listens on `.ei-trigger`, not the `<li>`, so `.ei-see-more` inside the panel needs no `stopPropagation` and opening the gallery cannot collapse the row.
...
- `#flash` is the mirror of `#gifts`: copy on the `--pad` rail, the Instagram panel flush to the right gutter. `justify-content: space-between` (not `flex-start`) is what puts it there — the copy column is capped at 640px, so the slack used to pool to the right of the panel and leave it floating 276px short of the gutter at 1920 — and `align-items: center` (not `flex-start`) because the panel is roughly half the copy column's height. The ≤1024px stack re-declares `align-items: stretch`, or the copy column would shrink-wrap and leave the rail.
