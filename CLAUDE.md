...
- **The enquiry form is the ask, and it opens a WhatsApp thread.** `.bk-form` is eight cards, one question each — name, WhatsApp number, idea, placement + size, budget, photos, when suits + email, review — because every DM used to arrive as "hi how much for". On send the browser composes the picked photos into **one** JPEG and POSTs everything to `POST /api/enquiry`, a Cloudflare Pages Function in this repo (`functions/api/enquiry.js`), which sends it **to the client's WhatsApp from Emmy's work number** as a UTILITY template. Emmy's number runs Meta Coexistence, so that message lands in her ordinary WhatsApp Business app as an outgoing message in a real chat with that client: one thread, both parties, photos attached, nothing to open. There is exactly **one** send button — a second route was the old design and the reason photos used to arrive as a list of links.
...
- **The WhatsApp token never touches the browser.** Everything secret lives in Pages env bindings read through `context.env`: `WA_TOKEN`, `WA_PHONE_ID`, `WA_TEMPLATE_PHOTOS`, `WA_TEMPLATE_PLAIN`, `WA_LANG`, `TURNSTILE_SECRET`, `ALLOWED_ORIGIN`, plus `GRAPH_BASE`/`TURNSTILE_URL` overrides that exist only so `scripts/graph-stub.js` can stand in for Meta during tests. The page holds one public value, `TURNSTILE_SITEKEY`. Empty sitekey means no widget and no token, which pairs with the function skipping Turnstile when its secret is unset — that pairing is the only reason `wrangler pages dev` is drivable headlessly, so do not make either side unconditional.
- **Template parameters are whitespace-collapsed, never empty.** Meta rejects a send outright if any parameter contains a newline, a tab or more than four consecutive spaces, and empty strings are refused too, so `p()` collapses all whitespace and substitutes an em dash. That is why the multi-line "idea" textarea arrives as a single line. The template body carries the line breaks itself; the variables never do.
- **Photos become one image, built in the browser.** A WhatsApp template carries exactly one header image, and four separate template messages would cost four times as much and arrive as four notifications. So `collage()` draws up to four photos into a single canvas — 1600px long edge for one photo, otherwise a 2-up grid of 1000px cells with 12px gutters on `#111111`, each photo `contain`-fitted because a cropped reference photo is a broken reference photo — then walks a JPEG quality ladder (`.85 → .72 → .6 → .5`) until the blob is under 1.4 MB. Measured: two 3:4 photos land at 2036×1024 and ~206 KB. Decoding goes through `<img>`, not `createImageBitmap`, because browsers apply EXIF orientation to `<img>` by default and a phone photo otherwise arrives sideways.
- **The card transitions are CSS-only, deliberately.** `.bk-card` animates with a keyframe and `.bk-card[hidden] { display: none }` is restated because `display: grid` on the same element outranks the UA `[hidden]` rule — the same trap `#pw-gate` hits on `/portfolio`. GSAP comes from a CDN and `__boot` returns at its lib guard when that fails; a form whose steps depended on GSAP would strand the visitor mid-ask. `#bkf-done` is a `.bk-card` **without** `data-step`, which is what keeps it out of the eight-card count and out of `Next`'s reach.
