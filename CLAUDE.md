...
- **`FIELDS` in the enquiry script is the single source of truth** for keys, caps and labels, and the message builder is shared, which is what makes the two variants produce a byte-identical message. Caps are sized against a real limit: WhatsApp accepts an 8000-character URL but **truncates prefilled text in the compose box above roughly 1000 characters**, so the values total 810 and the scaffold 146 — worst case 956. The `MSG_MAX` slice in `waMessage()` is defensive only; if you add a field, recompute the budget rather than leaning on it. Whitespace is collapsed to single spaces so every field stays on one labelled line, and empty optionals are dropped entirely rather than filled with a placeholder.
- **Two form variants ship in the same document.** A is a single page, B is the stepped card flow, both under `[data-variant]` with identical `name` attributes. A pre-paint IIFE in `<head>` resolves `?form=a|b` → `sessionStorage` → the `DEFAULT` constant (one line, the only thing to change to flip what real visitors get) and writes `data-form` on `<html>`; a CSS rule keyed off it hides the loser before first paint, and the enquiry script then **removes** it from the DOM. Removal, not hiding: a hidden field is still submitted, still read by `readFields`, and still reachable by keyboard and screen reader. Both `?form=a` and `?form=b` stay reachable in production and nothing in the UI hints that variants exist.
...
- **The card transitions are CSS-only, deliberately.** `.bk-card` animates with a keyframe and `.bk-card[hidden] { display: none }` is restated because `display: grid` on the same element outranks the UA `[hidden]` rule — the same trap `#pw-gate` hits on `/portfolio`. GSAP comes from a CDN and `__boot` returns at its lib guard when that fails; a form whose steps depended on GSAP would strand the visitor mid-ask. The success card (`#a-done` / `#b-done`) is a `.bk-card` **without** `data-step`, which is what keeps it out of the eight-card count and out of `Next`'s reach.
