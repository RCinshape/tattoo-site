...
- `#gifts` is a two-column row: copy on the `--pad` rail, the voucher artwork right, `rotate(8deg)` (`5deg` and centred at ≤1024px). `#gifts` keeps `overflow: hidden` because a rotated element paints wider than its layout box. The card's four edges dissolve via two `mask-image` gradients under `mask-composite: intersect`; it carries **no stroke** — a perimeter border sits exactly where the mask is transparent, and inset far enough to survive it reads as a frame printed across the artwork. It also carries no GSAP tween and no `opacity: 0`, and must stay out of the fallback lists, which would flatten its rotation with `transform: none !important`.
